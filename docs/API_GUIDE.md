...
> 상태: 구현 진행 중 · 상태 확인, 세션 인증, 재배 공간, 재배 시즌, 작물 기준정보, 텃밭 배치, 재배 일정, 시즌 기록 API 구현 완료
...
현재 공간·시즌·작물 기준정보·텃밭 격자·재배 일정·시즌 기록은 Laravel API와 데이터베이스를 사용한다. 물주기 일정·로그 등 남은 기능은 아직 브라우저 `localStorage`를 사용하므로, 이 문서는 구현된 계약과 다음 교체 범위를 함께 관리한다. 시즌 기록 API는 준비되었지만 기록 작성 화면은 아직 구현하지 않았다.
...
문서에 적었다고 기능이 구현된 것은 아니다. 현재 상태 확인, 세션 인증, 재배 공간·재배 시즌·텃밭 배치·재배 일정·시즌 기록 API가 구현되었고 나머지 엔드포인트는 **계획 상태**다.
...
| `GET` | `/crop-sources` | 작물 기준정보 출처 목록 | 불필요 |

작물 목록은 운영자가 관리하는 기준 데이터다. 일반 사용자는 이 API에서 수정하거나 삭제할 수 없다. 초기 13종 작물과 출처 4개는 작물 테이블 생성 마이그레이션에서 함께 등록되므로 별도 수동 입력이 필요하지 않다.
...
| 메서드 | 경로 | 설명 | 인증 |
|---|---|---|---|
| `GET` | `/layouts` | 내 시즌의 격자와 작물 배치 목록 조회 | 필요 |
...
대시보드처럼 여러 시즌의 배치를 한 번에 사용하는 화면은 시즌마다 요청을 반복하지 않고 `GET /layouts`를 사용한다. 목록에는 로그인 사용자가 소유한 시즌의 배치만 포함된다.
...
Laravel의 자동 생성은 텃밭 격자에 배치된 작물 종류마다 심기·파종 일정과 수확 시작 일정을 하나씩 만든다. 클라이언트는 마지막으로 읽은 배치 `version`을 생성 요청의 `If-Match`로 보내며, 서버는 현재 배치와 일치할 때만 일정을 교체한다. 예정일은 시즌 기간 안에서 작물의 월 단위 권장 시기와 겹치는 첫 날짜를 사용하며, `cropId`로 기준 작물과 연결한다. 같은 작물이 여러 칸에 배치되어도 일정은 중복 생성하지 않는다. 권장 시기가 시즌과 겹치지 않는 작물이 하나라도 있으면 기존 일정을 유지하고 전체 생성을 거부한다. 물주기 반복 일정은 별도 기능에서 정의한다.

`DELETE /seasons/{seasonId}/tasks`는 화면이 읽은 모든 일정의 `id`와 `version`을 함께 받는다. 서버의 현재 일정 집합과 하나라도 다르면 전부 유지하고 `412 VERSION_CONFLICT`를 반환하므로, 전체 삭제 중 일부만 사라지는 상태를 만들지 않는다. 일정이 남아 있는 시즌 삭제는 `409 SEASON_HAS_TASKS`로 거부한다.
...
기록의 `occurredAt`은 시간대가 포함된 ISO 8601 형식이어야 하며 시즌 시작일과 종료일 안에 있어야 한다. `quantity`와 `unit`은 둘 다 생략하거나 함께 보낸다. 기록 수정·삭제는 현재 `version`을 `If-Match`로 보내야 하며, 기록이 남은 시즌 삭제는 `409 SEASON_HAS_RECORDS`로 거부한다.
