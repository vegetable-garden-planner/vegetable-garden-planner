...
> 상태: 구현 진행 중 · 상태 확인, 세션 인증, 재배 공간, 재배 시즌, 작물 기준정보, 텃밭 배치, 재배 일정 API 구현 완료
...
현재 공간·시즌·작물 기준정보·텃밭 격자·재배 일정은 Laravel API와 데이터베이스를 사용한다. 기록 등 남은 기능은 아직 브라우저 `localStorage`를 사용하므로, 이 문서는 구현된 계약과 다음 교체 범위를 함께 관리한다.
...
문서에 적었다고 기능이 구현된 것은 아니다. 현재 상태 확인, 세션 인증, 재배 공간·재배 시즌·텃밭 배치·재배 일정 API가 구현되었고 나머지 엔드포인트는 **계획 상태**다.
