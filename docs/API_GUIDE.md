...
회원가입·로그인과 공간·시즌·격자 배치·재배 일정은 Laravel API를 호출해 데이터베이스에 저장한다. 작물 기준 데이터는 현재 프론트엔드 정적 데이터이며, 재배 기록·알림·관리자 API는 다음 구현 범위다. 이 문서는 다음 목적으로 사용한다.

- 구현된 핵심 API와 앞으로 추가할 API의 URL, HTTP 메서드와 데이터 구조를 합의한다.
- 프론트 타입과 백엔드 응답 이름이 달라지는 일을 줄인다.
- 정상 응답뿐 아니라 인증·검증·충돌·없음 오류를 미리 정의한다.
- Swagger UI, Postman과 자동 클라이언트 생성에 사용할 기반을 만든다.

`openapi.yaml`에는 구현된 핵심 API와 설계 중인 확장 API가 함께 있다. 실제 제공되는 경로의 최종 기준은 Laravel의 `routes/api.php`이며, 확장 API는 구현 상태를 확인한 뒤 사용해야 한다.
