...
- `growing_seasons.featured_crop_id`는 nullable 작물 slug이며 `crops.id`를 외래키로 참조합니다.

핵심 구현 순서는 `인증 → 재배 공간 → 재배 시즌 → 작물 기준정보 → 작물 배치 → 일정·기록`입니다. 각 단계는 마이그레이션, 모델 관계, API, 실패 경로 테스트까지 함께 완료합니다.
