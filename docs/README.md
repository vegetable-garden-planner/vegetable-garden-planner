...
1. 회원가입부터 기록까지 핵심 사용자 흐름 통합 점검
2. 닷홈 상품 사양 확인과 운영 배포 리허설
3. 사진 업로드와 시즌 마감 요약
4. 스케줄러·큐·이메일 또는 푸시 알림

GitHub Actions 검증은 `.github/workflows/ci.yml`에서 `develop` 대상 PR과 `develop` 푸시마다 자동 실행합니다.

관리자 작물 편집, 공동 텃밭, 결제, 사진 분석은 핵심 재배 경험과 배포가 안정된 뒤 진행합니다. 최신 세부 상태는 [PROJECT_STATUS.md](PROJECT_STATUS.md)를 봅니다.
