...
물주기 일정·완료·미루기·이력 디자인 개편은 `9ad46af`에서 완료했습니다. 시즌·배치·일정 현황을 보여 주는 상단 요약, 일정별 다음 물주기·간격·활성 상태 카드, 등록 사이드바, 완료·미루기 폼과 이력을 실제 인증 데이터로 데스크톱과 390px 모바일에서 확인했습니다. 화분·베란다는 격자나 배치 작물을 요구하는 화면을 보여 주지 않고 재배 일정 또는 시즌 기록으로 안내합니다.

시각 QA 중 로컬 Laragon DB에 이미 저장소에 존재하던 미적용 마이그레이션 `2026_08_13_060000_make_growing_space_region_optional`을 적용했습니다. QA 전용 사용자와 연관 데이터, 임시 모바일 확인 라우트는 검증 후 모두 제거했습니다. 새 마이그레이션이나 백엔드 변경은 이번 작업에 포함하지 않았습니다.

현재 로컬에서 이어받아야 할 미완성 디자인 변경은 없습니다. 진행 중이거나 막힌 작업과 다음 우선순위는 [작업 목록](TASKS.md)만 기준으로 확인합니다. 다음 디자인 영역은 시즌 재배 기록 목록·등록·수정 화면이며 아직 브랜치를 만들거나 구현을 시작하지 않았습니다.
