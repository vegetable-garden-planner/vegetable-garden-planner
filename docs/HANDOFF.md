...
시즌 재배 기록 목록·등록·수정 디자인 개편은 `93c2835`에서 완료했습니다. 시즌·공간·기록 현황 요약, 종류별 필터, 시간순 기록 타임라인, 등록 폼과 작성 가이드, 카드 내부 수정·삭제 확인을 실제 인증 데이터로 데스크톱 1280px과 모바일 390×844px에서 확인했습니다. 등록·필터·수정·삭제 확인 취소, 기록 있는 상태와 빈 상태를 점검했고 브라우저 오류는 없었습니다. 화분·베란다에서는 텃밭 전용 작물 배치 링크를 노출하지 않습니다.

브라우저 QA 전용 사용자와 연관 공간 1개·시즌 2개·기록 5개는 검증 후 모두 제거했습니다. 새 마이그레이션이나 백엔드 변경은 이번 작업에 포함하지 않았습니다.

현재 로컬에서 이어받아야 할 미완성 디자인 변경은 없습니다. 진행 중이거나 막힌 작업과 다음 우선순위는 [작업 목록](TASKS.md)만 기준으로 확인합니다. 다음 디자인 영역은 작물 목록·검색·필터와 작물 상세 화면이며 아직 브랜치를 만들거나 구현을 시작하지 않았습니다.
