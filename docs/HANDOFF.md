...
- 안정 기준 브랜치: `develop`, 최신 커밋 `e8b4f24`(2026-08-26) — 정확한 커밋은 작업 시작 시 `git log -1 --oneline develop`로 확인
