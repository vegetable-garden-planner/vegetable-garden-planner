...
- 재배 도우미 챗봇(2026-08-26 intent 기반, 2026-08-28 자유 텍스트 추가): 백엔드만 완성, 프론트 연결 화면은 아직 없음(팀원이 새 홈 화면과 함께 만들 예정). 두 경로가 공존한다 — (1) `POST /seasons/{season}/assistant/ask`: 화면 칩 전용, seasonId 필수, intent 4종 중 하나를 정확히 받아야 함(`log_watering`은 실제로 물주기를 완료 처리). (2) `POST /ai/chat`: 팀원이 검색창용으로 요청한 자유 텍스트 경로, `{"message": "..."}` → `{"data": {"answer": "..."}}`. seasonId 없이 로그인 사용자의 진행 중(또는 최신) 시즌을 서버가 자동으로 고르고, `ClassifyAssistantQuestion`(키워드 매칭)으로 메시지를 intent에 매핑해 (1)의 액션을 재사용한다. 아직 실제 LLM 연동이 아니며, 데이터를 바꾸는 `log_watering`은 오분류 위험 때문에 이 경로에서 자동 분류하지 않는다. 상세는 `TASKS.md` 최근 완료와 `openapi.yaml`의 `AiChat` 태그 참고. 실제 자유 텍스트 LLM으로 넓히게 되면 `GET /growing-context`(2026-08-25 완성)를 컨텍스트로 재사용하는 쪽을 먼저 검토한다.
