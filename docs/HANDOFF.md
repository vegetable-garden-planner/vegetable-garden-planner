...
작물 목록·검색·필터와 작물 상세 디자인 개편은 `0db50e5`에서 완료했습니다. 목록을 온실 사진 히어로, 13종 기준정보 현황, 이름·특징 검색, 공간 선택, 종류 캡슐 필터, 3열 이미지 카드, 공식 자료 영역의 긴 세로 구조로 재편했습니다. 상세는 작물 비주얼과 핵심 정보, 시작·간격·수확 단계, 꽃·실내 식물 관리 가이드, 시즌 시작 CTA, 공식 출처를 실제 API 데이터로 표시합니다. 잘못 연결되어 있던 시금치의 바질 사진은 제거했고, 전용 사진이 없는 작물은 카테고리 색과 한글 첫 글자를 사용합니다.

로컬 Laravel API와 Next.js를 연결해 데스크톱 1280px과 모바일 390×844px에서 전체 13종, 꽃 3종 필터, 결과 없음과 초기화, 상추 일반 상세, 호접란 관리 가이드, 외부 출처 표시를 확인했습니다. 가로 넘침과 브라우저 오류는 없었습니다. 공개 작물 화면이어서 QA 사용자나 임시 DB 데이터는 만들지 않았고, 새 마이그레이션이나 백엔드 변경도 포함하지 않았습니다.

현재 로컬에서 이어받아야 할 미완성 디자인 변경은 없습니다. 진행 중이거나 막힌 작업과 다음 우선순위는 [작업 목록](TASKS.md)만 기준으로 확인합니다. 다음 디자인 영역은 로그인·회원가입 화면 최종 시안 정렬이며 아직 브랜치를 만들거나 구현을 시작하지 않았습니다.
