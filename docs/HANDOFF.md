...
시작 진단과 요금제 화면으로 디자인 개편 사이클을 마쳤습니다.

시작 진단은 `AppPageShell`의 온실 사진 히어로 아래에 단계형 질문 카드를 두고, 결과에서 추천 공간·햇빛 안내·추천 식물·준비물·공간 등록 CTA를 순서대로 보여 줍니다. 가장 큰 변화는 추천 방식입니다. 이전에는 기준정보에 없는 식물 이름 21종을 하드코딩해 결과가 실제 데이터와 끊겨 있었습니다. 지금은 `getRecommendation(answers, crops)`가 작물 기준정보를 받아 공간·목적으로 거르고 관리 시간으로 난이도를 제한한 뒤 쉬운 순서로 최대 3종을 고르며, 각 추천은 `/crops/<slug>` 상세로 연결됩니다.

기준정보에 일조 요구량 필드가 없으므로 햇빛 답변은 작물을 거르지 않고 안내 문구로만 사용합니다. 이를 필터로 쓰려면 crops 테이블과 API, OpenAPI를 함께 바꿔야 하므로 별도 기능 작업으로 다룹니다.

실데이터에는 두 가지 구멍이 있습니다. 실내를 지원하는 기준정보는 꽃다발·호접란·아프리칸 바이올렛 3종뿐이라 실내에서 먹을 수 있는 작물이 없고, 마당을 지원하는 꽃은 하나도 없습니다. 이런 조건에서는 빈 화면 대신 대안 패널로 다른 공간과 그 공간에서 시작할 수 있는 식물, 해당 공간 등록 CTA를 안내합니다. 공간에는 맞지만 관리 시간과 어긋나는 경우에는 추천을 유지하고 주의 문구만 덧붙입니다. 36개 답변 조합을 모두 확인했고 빈 결과가 나오는 조합은 없습니다.

요금제는 별도 페이지 배경과 랜딩 헤더를 걷어내고 `AppPageShell` compact 히어로로 옮겼습니다. 혜택마다 `지금 제공`과 `서버 구현 후 제공` 배지를 붙이고, 프로에는 누를 수 없는 버튼 대신 안내 문구를 둡니다. 어디에서도 강제되지 않던 `evaluatePlanLimit`과 요금제 한도 정의는 화면만으로 기능을 흉내 내지 않는다는 기준에 따라 제거했습니다.

새 마이그레이션과 API 변경은 없습니다. 프론트 전체 검증(`npm test`, `typecheck`, `lint`, `build`)을 통과했습니다.

현재 로컬에서 이어받아야 할 미완성 변경은 없습니다. 진행 중이거나 남은 작업은 [작업 목록](TASKS.md)만 기준으로 확인합니다.
