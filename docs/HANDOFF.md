...
- 재배 도우미 챗봇(2026-08-26): 백엔드만 완성(`POST /seasons/{season}/assistant/ask`, 상세는 `TASKS.md` 최근 완료 참고). 프론트는 팀원이 새 홈 화면 디자인과 함께 만들 예정이라 이 저장소에는 아직 연결하는 화면이 없다. 나중에 자유 텍스트 질문으로 범위가 넓어지면 `GET /growing-context`(2026-08-25 완성)를 컨텍스트 조회에 재사용하는 쪽을 먼저 검토한다.
