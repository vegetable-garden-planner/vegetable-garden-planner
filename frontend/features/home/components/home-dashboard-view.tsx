...
export function HomeDashboardView(props: HomeDashboardViewProps) {
  return (
    <main className={styles.page}>
      <HeroSection {...props} />
      {props.dataError && <div className={styles.dataError} role="alert">{props.dataError}</div>}
      <GardenStatus {...props} />
      <TodayTasks {...props} />
      <GrowthSummary {...props} />
...
function HeroSection({
  ...
  const taskCountLabel = resourcesLoading
    ? "해야 할 일을 불러오는 중"
    : `오늘까지 해야 할 일 ${model.todayTaskCount}개`;
    ...
    <section className={styles.hero} aria-labelledby="hero-title">
      ...
      <div className={styles.heroInner}>
        <div className={styles.heroCopy}>
          ...
          <h1 id="hero-title">
            {authenticated
              ? <>오늘도<br />작은 텃밭은<br />자라고 있어요</>
              : <>나에게 맞는<br />작은 텃밭을<br />시작해 보세요</>}
          </h1>
...
function HeroDescription({
  ...
  if (!authenticated) return <>공간과 햇빛을 등록하면<br />나만의 재배 일정이 이어져요.</>;
  if (resourcesLoading) return <>{nickname}님의 재배 데이터를<br />불러오는 중이에요.</>;
  return <>{nickname}님이 키우는 작물 {cropCount}개가<br />서버에 안전하게 저장되어 있어요.</>;
}
...
function TodayTasks({ authenticated, model, resourcesLoading }: HomeDashboardViewProps) {
  return (
...
function GrowthSummary({ model, resourcesLoading }: HomeDashboardViewProps) {
  return (
...
function Recommendations({ authenticated, model }: HomeDashboardViewProps) {
  return (
