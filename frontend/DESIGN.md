...
기준일: 2026-08-19. 현재 대시보드, 작물 배치 상세, 재배 일정, 재배 공간 목록·등록·수정, 시즌 목록·등록·수정, 물주기 일정·완료·미루기·이력, 시즌 재배 기록 목록·등록·수정, 작물 목록·검색·필터·상세, 로그인·회원가입, 시작 진단·요금제, 로고 전면 교체, 공통 빈 상태·오류·로딩·삭제 확인·모바일 내비게이션까지 이 디자인 시스템을 적용했다. 정확한 진행 상태와 다음 순서는 [작업 목록](../docs/TASKS.md)을 기준으로 확인한다.
...
- App header: 내부 화면에서는 화면 폭 전체의 흰색 반투명 상단 바와 얇은 하단 경계를 사용한다. 브랜드 로고는 `public/brand/logo.png`, 중앙 메뉴는 짙은 회색, 검색과 계정 작업은 오른쪽에 둔다. 대시보드처럼 사진 위에 겹치는 `overlay` 헤더는 대비를 위해 `logo-white.png`를 사용한다. 모바일 내비게이션은 한 줄 스크롤을 유지하되 스크롤바는 노출하지 않는다.
...
- Auth page: 데스크톱에서는 온실 사진과 폼을 1:1로 나눈 패널, 모바일에서는 사진을 숨기고 폼을 우선 노출한다. 이미지 위 문구에는 overlay를 두며 로그인에는 Google·카카오 버튼, 이메일 로그인 구분선, 제공자별 실패 안내를 표시한다. 회원가입의 이메일 중복 확인·비밀번호 조건·필수 동의 동작은 스타일 개편 후에도 그대로 유지한다.
...
- Empty/loading/error: empty는 surface 배경과 설명+CTA, loading은 surface/primary-soft skeleton, error는 `--color-danger` 토큰 텍스트와 재시도 CTA. 삭제·초기화처럼 되돌릴 수 없는 동작은 브라우저 기본 `confirm()` 대신 `components/inline-confirm.tsx`(전역 `.inline-confirm` 클래스)로 대상 카드·버튼 자리에서 바로 확인한다.
- App/home header 모바일: 화면 폭이 좁아지면 메뉴를 숨기거나 동작하지 않는 아이콘으로 대체하지 않고, 헤더가 두 줄로 감싸며 내비게이션 행이 가로 스크롤되도록 한다(`app-header`, 홈 화면 헤더 공통 패턴).
...
- Start diagnosis: 시작 진단은 온실 사진 히어로 아래에 한 번에 한 질문을 보여 주는 흰색 카드와 진행 막대를 둔다. 결과는 추천 공간·햇빛 안내·추천 식물 카드·준비물·공간 등록 CTA 순서로 쌓는다. 추천 식물은 하드코딩한 이름이 아니라 작물 기준정보에서 고르고 각 카드는 작물 상세로 연결한다. 조건에 맞는 기준정보가 없으면 빈 화면 대신 따뜻한 배경의 대안 패널로 다른 공간과 시작 가능한 식물을 안내한다.
- Plans: 요금제는 별도 페이지 배경이나 랜딩 헤더를 만들지 않고 `AppPageShell`의 compact 히어로를 사용한다. 무료는 흰색 카드, 프로는 짙은 숲색 그라디언트 카드로 구분하고 각 혜택에는 `지금 제공`과 `서버 구현 후 제공` 배지를 붙인다. 결제가 없는 상태이므로 프로에는 비활성 버튼 대신 안내 문구를 둔다.
