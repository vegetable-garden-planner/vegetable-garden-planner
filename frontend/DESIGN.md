...
기준일: 2026-08-18. 현재 대시보드, 작물 배치 상세, 재배 일정, 재배 공간 목록·등록·수정 화면까지 이 디자인 시스템을 적용해 `develop`에 병합했다. 시즌 목록·등록·수정은 `feature/codex-growing-season-design-refresh`에서 진행 중이며, 이후 물주기 → 시즌 기록 → 작물 목록·상세 → 인증 → 시작 진단·요금제 순서로 적용한다. 세부 파일과 남은 검증은 `docs/HANDOFF.md`를 기준으로 확인한다.
