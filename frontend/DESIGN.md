...
기준일: 2026-08-18. 현재 대시보드, 작물 배치 상세, 재배 일정, 재배 공간 목록·등록·수정, 시즌 목록·등록·수정, 물주기 일정·완료·미루기·이력, 시즌 재배 기록 목록·등록·수정 화면까지 이 디자인 시스템을 적용했다. 다음 디자인 작업은 작물 목록·상세 → 인증 → 시작 진단·요금제 순서로 적용한다. 정확한 진행 상태와 다음 순서는 [작업 목록](../docs/TASKS.md)을 기준으로 확인한다.
